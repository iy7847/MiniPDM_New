-- 1. order_items 테이블에 due_date (납기일) 및 work_days (견적소요일) 컬럼 추가 (프론트엔드와 싱크)
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS due_date DATE;
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS work_days INTEGER;

-- 2. 기존 수주 전환 RPC 덮어쓰기 (due_date 계산 로직 및 work_days 복사 포함)
...
BEGIN
    ...
    LOOP
        ...
        -- 견적 소요일(work_days)을 오늘 날짜에 더해서 납기일(due_date) 기본값 계산
        -- work_days가 없거나 0이면 기본 3일 후로 세팅
        v_due_date := CURRENT_DATE + (COALESCE(NULLIF(v_item.work_days, 0), 3) || ' days')::interval;
