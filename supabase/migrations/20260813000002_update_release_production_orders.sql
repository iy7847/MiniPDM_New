...
-- 1:1 Material Order architecture update
...
BEGIN
    ...
    FOR v_item IN 
        SELECT oi.id, oi.supply_type, oi.material_supply_type, oi.production_status, oi.part_name, oi.material_name, oi.qty, oi.production_qty, oi.estimate_item_id, oi.order_id, oi.order_item_no
        FROM order_items oi
        ...
        IF v_item.production_status IS NULL OR v_item.production_status = 'PENDING' THEN
            ...
            IF v_item.material_supply_type = 'ORDER' THEN
                IF NOT EXISTS (SELECT 1 FROM material_orders WHERE order_item_id = v_item.id) THEN
                    -- Insert into material_orders directly with M-prefixed po_no
                    -- 1품목 1소재발주 아키텍처 적용: material_order_items 연결 테이블 사용 중단
