-- ==============================================================================
-- [MiniPDM v2.0] 수주 전환 및 수주번호 중복 에러 해결 마이그레이션 스크립트
-- 
-- 1. orders 테이블의 전역 UNIQUE 제약조건(orders_order_number_key) 제거
-- 2. 회사별 복합 UNIQUE 제약조건(company_id, order_number) 추가 (테넌트 격리)
-- 3. convert_estimate_to_order 수주 전환 RPC 함수 채번 로직 강화
-- ==============================================================================

-- 1. 기존 전역 단일 UNIQUE 제약조건 제거 (다른 회사와의 수주번호 충돌 해제)
ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS orders_order_number_key;
ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS orders_po_no_key;

-- 2. 회사별 복합 고유 제약조건 생성 (회사 내에서만 수주번호 중복 방지)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'orders_company_order_number_uk'
    ) THEN
        ALTER TABLE public.orders ADD CONSTRAINT orders_company_order_number_uk UNIQUE (company_id, order_number);
    END IF;
END $$;

-- 3. convert_estimate_to_order RPC 함수 교체 (회사별 채번 충돌 방어 강화)
...
BEGIN
    ...
    -- 2. 새 수주 번호(PO) 채번: P{YYMM}-{XXX} (회사별 중복 충돌 자동 회피 루프)
    ...
    LOOP
        ...
        -- 회사 내 중복 검사 (po_no 및 order_number 동시 점검)
        IF NOT EXISTS (
            SELECT 1 FROM public.orders 
            WHERE company_id = p_company_id 
              AND (po_no = v_po_no OR order_number = v_po_no)
        ) THEN
            EXIT;
        END IF;
