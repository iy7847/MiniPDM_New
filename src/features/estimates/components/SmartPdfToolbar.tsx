    ...
    <div className="bg-bg-elevated p-2 border-b border-border-default flex flex-wrap gap-2 items-center z-10 shadow-none w-full min-h-[50px]">
      ...
      <div className="flex items-center gap-1 border-r border-border-default pr-2">
        ...
        <div className="flex gap-0.5">
          ...
          <button onClick={() => changePage(1)} disabled={pageNumber >= (numPages || 1)} className="w-9 h-7 border border-border-default rounded hover:bg-bg-surface flex items-center justify-center text-text-secondary text-xs" title="다음 페이지 (방향키 ▶)">▶</button>
        </div>
      </div>
        ...
        {!isViewerOnly && (
          <>
            ...
            <button
              ...
              className="px-3 py-1.5 text-xs rounded border border-border-default flex items-center gap-1 bg-bg-elevated text-text-secondary hover:text-status-danger hover:border-status-danger/30 transition-all"
              title="단축키: E"
            >
              <span>🗑️</span>
              <span>지우기 <span className="opacity-50">(E)</span></span>
            </button>
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center border border-border-default rounded overflow-hidden">
          <button onClick={() => setScale((s: number) => Math.max(0.1, s - 0.1))} className="px-2 py-1 hover:bg-bg-surface text-text-secondary text-xs">－</button>
          <span className="text-xs font-mono w-10 text-center bg-bg-surface py-1 border-x">{Math.round(scale * 100 * (1 / 0.25))}%</span>
          <button onClick={() => setScale((s: number) => Math.min(2.0, s + 0.1))} className="px-2 py-1 hover:bg-bg-surface text-text-secondary text-xs">＋</button>
        </div>

        {onReset && (
          <button
            onClick={onReset}
            className="px-3 py-1.5 text-xs font-bold text-status-danger border border-border-default hover:bg-status-danger/10 rounded transition-colors"
          >
            초기화
          </button>
        )}
